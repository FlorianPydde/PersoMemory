...
Use `memory` for ambiguous, mixed, scoped recall, capture, commitment, and focus requests. Use `memory-brief` for broad day-level focus. Use `memory-sweep` for WorkIQ or Copilot evidence intake. Use `memory-maintenance` for consolidation, stale review, archive, merge, supersede, and cleanup.
